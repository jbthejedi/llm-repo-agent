...
We extracted ~1,600 training samples from teacher traces, significantly below the expected ~2,300. This shortfall occurred because fewer rollouts reached successful completion than anticipated.
...
| Errored Runs | 8/15 (53%) | 10/15 (67%) | ↑ worse |
| Total Parse Errors | 8 | 10 | ↑ worse |
| Tool Parse Success Rate | 94.2% | 86.1% | ↓ 8% |
| Avg Steps per Run | 8.8 | 4.5 | ↓ (fails earlier) |
| Total Valid Tool Actions | 129 | 62 | ↓ 52% |

The SFT model performed worse than baseline across every metric. Most critically, the tool parse success rate—the primary target of this experiment—dropped from 94.2% to 86.1%.
...
1. **Insufficient training data**: ~1,600 samples is far below the 6,000–10,000 range typically needed for reliable behavior change. The model may have overfit to spurious patterns without learning the core constraint.

2. **No negative examples**: The training data only contained valid tool calls from successful rollouts. The model never learned what *not* to do—it never saw the "prose before JSON" failure mode and learned to avoid it.

3. **Verbosity inheritance**: The teacher model (72B) may have included reasoning in its outputs. Even if parsed successfully, this could have taught the student to be more verbose.

4. **Baseline wasn't broken**: The baseline already achieved 94.2% tool parse success. There wasn't much room to improve, and the narrow SFT may have disrupted existing capabilities (catastrophic forgetting).
...
1. **Scale up training data**: Target 6,000–10,000 samples by expanding to all 42 QuixBugs tasks, increasing rollouts per task, and relaxing `--require-success` to include good-prefix steps from failed rollouts.

2. **Add negative examples**: Include examples of the failure mode (prose before JSON) with corrected outputs, or use DPO to contrast good vs. bad outputs.

3. **Audit teacher outputs**: Ensure the training data contains *only* pure JSON responses, no reasoning preambles.

4. **Consider DPO**: With preference pairs (good JSON vs. prose+JSON), the model can learn the contrast directly rather than only imitating positive examples.