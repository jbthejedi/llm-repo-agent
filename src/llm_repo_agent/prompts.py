...
def system_prompt(tool_protocol: str = "native") -> str:
  ...
  sections = [
    ...
    "- Your first tool call must be list_files with rel_dir='.' (repo root), even if the goal names a specific file.",
    "- Do not call read_file/grep/write_file before this root listing.",
    "- list_files must include both rel_dir and max_files (use max_files=200 unless you have a reason to choose a smaller number).",
    "",
    "FINAL OUTPUT (STRICT):",
    ...
    "- For determining test commands after the initial root listing, if unclear, grep for:",
    "pytest, package.json, pyproject.toml, requirements, Makefile, setup.cfg, tox.ini",
